...
class_name TweenMovingPlatform
...
func setup_points():
	...
	for i in range(len(targets) - 1):
		var point: TargetDistanceTime = TargetDistanceTime.new(
	...
	# Append a TargetDistanceTime from last to first point as well, so that the platform moves back to the starting location.
	var last_point: TargetDistanceTime = TargetDistanceTime.new(
		targets[targets.size() - 1].global_position,
		targets[0].global_position,
		speed
	)
	_target_points.append(last_point)
...
func set_moving() -> void:
	...
	# The docs tells to put a very short interval when looping tweens.
